...
export class FollowPlanServer {
  ...
  private createPlanStructure(): PlanStructure {
    const planDir = path.join(this.projectPath, '.plan');
    return {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          ...
          case 'delete_message':
            return messageHandlers.deleteMessage(validatedArgs as { id: string });
          case 'get_message':
            return messageHandlers.getMessage(validatedArgs as { id: string });
          case 'list_messages':
            return messageHandlers.listMessages(validatedArgs as any);
          case 'get_thread':
            return messageHandlers.getThread(validatedArgs as { threadId: string; limit?: number; offset?: number });
          case 'get_replies':
            return messageHandlers.getReplies(validatedArgs as { messageId: string });
          case 'add_reaction':
            return messageHandlers.addReaction(validatedArgs as { messageId: string; reaction: string; userId: string });
          case 'remove_reaction':
            return messageHandlers.removeReaction(validatedArgs as { messageId: string; reaction: string; userId: string });
          case 'add_related_item':
            return messageHandlers.addRelatedItem(validatedArgs as { messageId: string; itemType: "task" | "feature" | "bug" | "rule"; itemId: string; itemTitle: string });
          case 'remove_related_item':
            return messageHandlers.removeRelatedItem(validatedArgs as { messageId: string; itemType: string; itemId: string });
          case 'get_mentions':
            return messageHandlers.getMentions(validatedArgs as { userId: string; limit?: number; offset?: number });
          case 'get_thread_summary':
            return messageHandlers.getThreadSummary(validatedArgs as { threadId: string });
  ...
  async initialize(): Promise<void> {
    ...
    try {
      logger.info(`Initializing Follow Plan MCP Server for project: ${this.projectPath}`);
